...
import { isDate, assertIsNotDate, assertIsDate, assertIsDateOrNull } from '../../../src';
...
describe("date checker", () => {
  testIs(isDate, baseTests);
});

describe("date assertions", () => {
  testAssertIs(assertIsDate, baseTests);
  testAssertIsNot(assertIsNotDate, baseTests);
  testAssertIsOrNull(assertIsDateOrNull, baseTests);
});
